...
import QuoteProgress from "@/components/molecules/QuoteProgress";
import QuoteStepHeader from "@/components/molecules/QuoteStepHeader";
...
const QuoteForm = () => {
  ...
  const handleSendQuote = async () => {
    ...
    try {
      ...
      toast.error(
        "Hubo un error al enviar la solicitud. Intenta por WhatsApp.",
        { id: toastId },
      );
    } finally {
      setLoading(false);
  ...
  const openWhatsApp = () => {
    ...
    const waMessage =
      `*Nueva Solicitud de Cotización - Tupla Core*%0A%0A` +
        ...
        <AnimatePresence mode="wait">
          ...
          {step === 1 && (
            <motion.div key="step1" {...fadeInUp} className="space-y-8">
              <QuoteStepHeader
                title="Que necesitas construir?"
                description="Selecciona el tipo de solucion que buscas."
                center
              />
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {services.map((s) => (
                  <ServiceCard
                    key={s.id}
                    {...s}
                    isActive={formData.service === s.id}
                    onClick={(id) => {
          ...
          {step === 2 && (
            <motion.div key="step2" {...fadeInUp} className="space-y-8">
              <QuoteStepHeader
                title="Cuentanos mas"
                description="Describe brevemente tu idea o los objetivos principales."
              />
          ...
          {step === 3 && (
            <motion.div key="step3" {...fadeInUp} className="space-y-8">
              <QuoteStepHeader
                ...
                description="Esto nos ayuda a priorizar y proponer tecnologias adecuadas."
              />

              <div className="grid md:grid-cols-2 gap-8">
                <div className="space-y-4">
                  <label className="text-[0.65rem] font-bold uppercase tracking-widest text-gray-500 flex items-center">
                    <DollarSign size={12} className="mr-2" /> Presupuesto
                    Estimado
                  </label>
                  <Select
                    onValueChange={(val) =>
                  ...
                  >
                    ...
                    <SelectContent className="bg-white dark:bg-tupla-dark border-white/10">
                      <SelectItem value="menos de $5'000.000">
                        Menos de $5&apos;000.000 COP
                      </SelectItem>
                      <SelectItem value="$5'000.000 - $15'000.000">
                        $5&apos;000.000 - $15&apos;000.000
                      </SelectItem>
                      <SelectItem value="$15'000.000 - $40'000.000">
                        $15&apos;000.000 - $40&apos;000.000
                      </SelectItem>
                      <SelectItem value="$40'000.000+">
                        $40&apos;000.000+
                      </SelectItem>
                    </SelectContent>
                  </Select>
                ...
                <div className="space-y-4">
                  ...
                  >
                    ...
                    <SelectContent className="bg-white dark:bg-tupla-dark border-white/10">
                      <SelectItem value="menos de 1 mes">
                        Urgente (menos de 1 mes)
                      </SelectItem>
                      <SelectItem value="2-4 meses">
                        Estándar (2-4 meses)
                      </SelectItem>
          ...
          {step === 4 && (
            <motion.div key="step4" {...fadeInUp} className="space-y-8">
              <QuoteStepHeader
                ...
                description="A quien dirigimos nuestra propuesta?"
                center
              />
